...
except (FileNotFoundError, KeyError, AttributeError): # Added AttributeError for st.secrets if not available
...
# Ensuring we use the Pro model as discussed
...
def create_user(username, password, user_type, college_name=None):
    ...
    try:
        ...
        conn.commit()
        print(f"[{datetime.now()}] User created: {username}")
        ...
        return False, f"An error occurred while creating user."
    finally:
        if cursor: cursor.close()
        if conn: conn.close()
...
def authenticate_user(username, password):
    ...
    try:
        conn = get_db_connection(); 
        if conn is None: st.error("Database connection failed during login."); return
        ...
        if user_record and check_password_hash(user_record['password_hash'], password):
            ...
            print(f"[{datetime.now()}] User {username} logged in. Session User ID: {st.session_state.current_user_id}")
            st.rerun()
        else:
            st.error("Invalid username or password")
    except (Exception, psycopg2.Error) as error:
        st.error(f"Authentication error. Please try again.")
        print(f"Auth Error for user {username}: {error}")
    finally:
        if cursor: cursor.close()
        if conn: conn.close()
...
def save_student_profile(user_id, full_name, department, branch, roll_number, email):
    if user_id is None:
        st.error("Cannot save profile: User ID is not available. Please log out and log in again.")
        return False
    sql = """
        ...
        ON CONFLICT (user_id) DO UPDATE SET
            ...
            roll_number = EXCLUDED.roll_number, email = EXCLUDED.email;
    """
    conn = None; cursor = None
    try:
        conn = get_db_connection(); 
        if conn is None: st.error("Failed to save profile: Database connection error."); return False
        cursor = conn.cursor()
        cursor.execute(sql, (user_id, full_name, department, branch, roll_number, email))
        conn.commit()
        print(f"[{datetime.now()}] Student profile saved/updated for user_id: {user_id}")
        return True
    except (Exception, psycopg2.Error) as error:
...
def save_essay_submission(student_user_id, title, content_markdown, ai_feedback_json_str, overall_rating):
    if student_user_id is None:
         ...
         return
    sql = "INSERT INTO essays (student_user_id, title, content_markdown, submission_time, ai_feedback_json, overall_rating) VALUES (%s, %s, %s, %s, %s, %s)"
    conn = None; cursor = None
    submission_time_val = datetime.now()
    try:
        conn = get_db_connection(); 
        if conn is None: st.error("Failed to save essay: Database connection error."); return
        cursor = conn.cursor()
...
def get_student_essays(student_user_id):
    if student_user_id is None: return []
    conn = None; cursor = None
    try:
        conn = get_db_connection(); 
        if conn is None: return []
        cursor = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        cursor.execute("SELECT id, title, content_markdown, submission_time, ai_feedback_json, overall_rating FROM essays WHERE student_user_id = %s ORDER BY submission_time DESC", (student_user_id,))
...
def get_college_reports(college_name): 
    ...
    sql_query = '''
        SELECT 
            ...
            sp.full_name as student_full_name, sp.department as student_department,
            sp.branch as student_branch, sp.roll_number as student_roll_number
        FROM essays e
    ...
    try:
        ...
        st.error("Failed to fetch college reports.")
        print(f"SQL Error in get_college_reports for {college_name}: {error}")
        return [] 
    finally:
        if cursor: cursor.close()
        if conn: conn.close()

# --- NEW: Fetch Students by College ---
def get_college_students(college_name):
    conn = get_db_connection()
    if conn is None:
        st.error("Database connection failed.") # Show error in UI
        return []
    cursor = None
    try:
        cursor = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        cursor.execute('''
            SELECT u.username, u.id as user_id, sp.full_name, sp.department, sp.branch, sp.roll_number, sp.email
            FROM users u
            LEFT JOIN student_profiles sp ON u.id = sp.user_id
            WHERE u.college_name = %s AND u.user_type = 'student'
        ...
        st.error("Failed to fetch student list for your college.")
        print(f"Error fetching college students for {college_name}: {e}")
        return []
    finally:
        if cursor: cursor.close()
        if conn: conn.close()
...
def get_gemini_assessment(title, essay_markdown):
    # ... (Unchanged Gemini Prompt & Parsing) ...
    prompt = f"""
...
def process_and_submit_essay(student_user_id, title, essay_content_html):
    if student_user_id is None:
         st.error("Cannot submit essay: User ID is not available. Please log out and log in again.")
         return
    if not title.strip():
        st.warning("Essay title cannot be empty for submission.")
        return
    essay_markdown = ""
    if essay_content_html and essay_content_html.strip() and essay_content_html != "<p><br></p>":
        try:
            essay_markdown = md(essay_content_html)
        except Exception as e_md:
            st.error(f"Error converting essay content to Markdown: {e_md}")
            essay_markdown = "<i>Error converting content.</i>" 
    else:
...
# --- UI Sections ---
with st.sidebar:
    # ... (Sidebar UI - UNCHANGED from previous enhanced version) ...
...
if not st.session_state.logged_in:
    # ... (Login/Signup UI - UNCHANGED from previous enhanced version) ...
        ...
        if st.session_state.view == 'login':
            ...
            with st.container(border=True): 
                ...
                with st.form("signup_form_main"):
                    ...
                    signup_college_name = st.text_input("Your College Name", key="signup_college_main", placeholder="e.g., Truskill University") 
                    st.markdown("<br>", unsafe_allow_html=True)
                    ...
                    if signup_submitted:
                        if signup_username and signup_password and signup_college_name:
                            if len(signup_password) < 6:
                                ...
                                if success:
                                    st.success(message + " Please log in to continue.") 
                                    st.session_state.view = 'login' 
                                    st.rerun() 
    ...
    if st.session_state.user_type == 'super_admin':
        # ... (Super Admin UI - UNCHANGED from previous enhanced version) ...
        ...
        # --- NEW: Expander for Viewing Registered Students ---
        with st.expander("👥 View Registered Students", expanded=False):
            if st.session_state.current_college_name:
                student_list = get_college_students(st.session_state.current_college_name)
                if not student_list:
                    st.info("No students currently registered for your college.")
                else:
                    df_students = pd.DataFrame(student_list)
                    # Select and rename columns for display
                    df_students_display = df_students[['full_name', 'username', 'department', 'branch', 'roll_number', 'email']].copy()
                    df_students_display.columns = ["Full Name", "Username", "Department", "Branch", "Roll Number", "Email"]
                    st.dataframe(df_students_display, use_container_width=True, hide_index=True)
            else:
                st.warning("College name not set for this admin account.")
        st.markdown("---")
        # --- END: New Student Viewer ---

        st.subheader("📊 Student Essay Reports") # Existing section
        # ... (Filter/Sort UI, Excel Export, Report Display - UNCHANGED from previous enhanced version) ...
        with st.container(border=True): 
        ...
        st.header(f"📝 Student Dashboard - {st.session_state.get('current_college_name', 'Your College')}") # Safer get
        
        if st.session_state.get('current_user_id') is None: # Use .get for safety
            st.warning("Your session might have expired or user ID is not set. Please log out and log back in to continue.")
            if st.button("Log Out to Refresh Session", key="student_logout_refresh_btn"):
                logout()
            st.stop() 
        ...
        if profile_incomplete: 
            ...
            # ... (Student: Start New Essay UI - UNCHANGED from previous enhanced version) ...
            with st.container(border=True):
            ...
            # ... (Student: Essay Writing UI with Quill - UNCHANGED from previous enhanced version) ...
            ...
            essay_html_content = st_quill(value=st.session_state.get("essay_content_html", ""), placeholder="Compose your brilliant essay here...", html=True, toolbar=toolbar_config_essential, key="quill_editor_main")
        ...
        if student_essays:
            # ... (Student: Past Submissions display with graphical report - UNCHANGED from previous enhanced version) ...
            st.markdown("---")
